...
def html_cleaner(html):
    ...
    return list(x.lower() for x in cleaned_sents.split(' '))
...
#     print(list(gs.translate(lines, 'en', 'fr')))
#     return gs.translate(lines, 'en', 'fr')

def dicTranslate(lines, translate_src_lang):
    logger.info('translate: %s lines', len(lines))
    return list((french_eng[x] for x in lines if x in french_eng.keys()))
    ...
    for line in lines:
        ...
        if part_name == 'WARC':
            ...
            if line == '</html>':
                ...
                # print(" cleaned html lines " + cleaned_html_lines)
                translated_cleaned_html_lines = (
                    list(dicTranslate(cleaned_html_lines, translate_src_lang))
